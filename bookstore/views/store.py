...
from api.sql import Cart, Member, Order_List, Product, Record
...
def bookstore():
    ...
    if "keyword" in request.args and "page" in request.args:
        ...
        cursor.execute(
            "SELECT * FROM PRODUCT WHERE PNAME LIKE %s", ("%" + search + "%",)
        )
        ...
        for i in book_row:
            book = {"商品編號": i[0], "商品名稱": i[1], "商品價格": i[2]}
    ...
    if request.method == "POST":
        if "pid" in request.form:
            ...
            pid = request.form.get(
                "pid"
            )  # 使用者想要購買的東西，使用 `request.form.get()` 來避免 KeyError
            if not pid:
                flash("Product ID is missing.")
                return redirect(
                    url_for("bookstore.cart")
                )  # 返回購物車頁面並顯示錯誤信息
            ...
            if product is None:
                Record.add_product(
                    {"pid": pid, "tno": tno, "saleprice": price, "total": price}
                )
            else:
                # 如果購物車裡面有的話，就多加一個進去
...
def orderlist():
    ...
    for j in orderdetail_row:
        temp = {"訂單編號": j[0], "商品名稱": j[1], "商品單價": j[2], "訂購數量": j[3]}
